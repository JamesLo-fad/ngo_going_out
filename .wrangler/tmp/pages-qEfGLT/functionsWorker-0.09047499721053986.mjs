...
var onRequestGet2 = /* @__PURE__ */ __name(async ({ env, request }) => {
  try {
    ...
    const orderBy = q ? "ORDER BY bm25(f) ASC, o.name COLLATE NOCASE ASC" : "ORDER BY o.name COLLATE NOCASE ASC";
...
// api/_archived/search_policies.js
var onRequestGet3 = /* @__PURE__ */ __name(async ({ env, request }) => {
  try {
    if (env.ALLOW_POLICY_SEARCH !== "1") return json2({ items: [], total: 0, page: 1, totalPages: 1 });
    const url = new URL(request.url);
    const q = (url.searchParams.get("q") || "").trim();
    ...
    const maxPages = parseInt(env.MAX_PAGES || "50", 10);
    if (page > maxPages) {
      return json2({ items: [], page, pageSize, total: maxPages * pageSize, totalPages: maxPages });
    }
    const clauses = [];
    const params = [];
    let from = "policies p";
...
// ../.wrangler/tmp/pages-qEfGLT/functionsRoutes-0.6711878792087447.mjs
var routes = [
  {
...
// ../../.nvm/versions/node/v24.4.1/lib/node_modules/wrangler/node_modules/path-to-regexp/dist.es2015/index.js
function lexer(str) {
  var tokens = [];
  var i = 0;
  while (i < str.length) {
...
// ../../.nvm/versions/node/v24.4.1/lib/node_modules/wrangler/templates/pages-template-worker.ts
var escapeRegex = /[.+?^${}()|[\]\\]/g;
...
// ../.wrangler/tmp/bundle-0djarl/middleware-insertion-facade.js
...
// ../.wrangler/tmp/bundle-0djarl/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.09047499721053986.mjs.map
