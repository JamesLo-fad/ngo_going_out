...
async function main() {
  ...
  for await (const line of rl) {
    ...
    const row = {
      ...
      in_cnie: parseYesNo(makeUmbrellaFlags({}, map, cols).includes('中促会')),
      in_cace: parseYesNo(makeUmbrellaFlags({}, map, cols).includes('民促会')),
      in_un: parseYesNo(makeUmbrellaFlags({}, map, cols).includes('联合国')),
      ...
      donation_pre: parseFloat(get(cols, map, '捐赠金额（出海前）标注年份') || get(cols, map, '捐赠金额（出海前）') || get(cols, map, 'donation_pre') || '0') || null,
      donation_pre_year: get(cols, map, '2008年') || get(cols, map, 'donation_pre_year') || '',
      donation_post: parseFloat(get(cols, map, '捐赠金额（出海后）') || get(cols, map, 'donation_post') || '0') || null,
      donation_post_year: get(cols, map, 'donation_post_year') || '',
